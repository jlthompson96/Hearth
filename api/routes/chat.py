...
from agents.tally import DoneEvent, TokenEvent, ToolEvent, ToolResultEvent, answer
...
def _stream(message: str, today: dt.date) -> Iterator[str]:
    try:
        for item in answer(message, today=today):
            match item:
...
@router.post("/chat", summary="Ask Tally a question; streams the answer as SSE")
def post_chat(request: ChatRequest) -> StreamingResponse:
    return StreamingResponse(
        _stream(request.message, request.today or dt.date.today()),
